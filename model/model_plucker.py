...
class conv_in_seq_direction_moment_knn(nn.Module):
    def __init__(self, out_channel: int, in_channel: int = 6):
        super().__init__()
        self.in_channel = in_channel  # 6 for standard Plücker, 9 for Plücker+RGB
        self.seq_out_channel = out_channel//2

        # x[:,:3,:] (first 3 dims, always moments) -> get_graph_feature -> 6D -> conv_direction
        direction_in_channels = 6  # 3D -> get_graph_feature -> 6D
        # x[:, 3:,:] (remaining dims, directions+colors) -> get_graph_feature -> (in_channel-3)*2 -> conv_moment
        moment_in_channels = (self.in_channel - 3) * 2
        
        self.conv_direction = torch.nn.Conv2d(direction_in_channels, self.seq_out_channel // 8, 1)
        self.conv_moment = torch.nn.Conv2d(moment_in_channels, self.seq_out_channel // 8, 1)
...
class FeatureExtractorGraph(nn.Module):
  ...
  def __init__(self, config, in_channel):
    ...
    self.conv_in = conv_in_seq_direction_moment_knn(self.config['net_nchannel'], in_channel=in_channel)
...
class PluckerNetKnn(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.in_channel = getattr(config, 'in_channel', 6)  # default 6 for standard Plücker
        # feature extractor
